...
@Component({
  ...
  templateUrl: './accounts-payable-aging.html',
  styleUrl: './accounts-payable-aging.scss',
})
export class AccountsPayableAging implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.stockLedgerService.getAccountsPayableAging().subscribe({
      next: (response) => {
      ...
      error: (error: HttpErrorResponse) => {
        this.loading.set(false);
        this.notificationService.error(this.extractErrorMessage(error, 'Unable to load AP aging.'));
      },
    });
  }
